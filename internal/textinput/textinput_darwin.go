...
func (t *textInputImpl) start(bounds image.Rectangle) (<-chan textInputState, func()) {
	...
	//
	// Not when this session follows a commit closely: an IME such as Korean
	// commits a syllable and marks the next jamo in one key press, and that
	// marked text, already queued, is this session's.
	if !t.events.carrying() {
