...
// Name sets the accessible name without adding a visible label.
func (s *ThemeSwitchWidget) Name(name string) *ThemeSwitchWidget { s.SetName(name); return s }
...
// BindDisabled follows a disabled-state binding without rebuilding.
func (s *ThemeSwitchWidget) BindDisabled(r ggui.Readable[bool]) *ThemeSwitchWidget {
	s.SwitchWidget.BindDisabled(r)
	return s
}
...
func (s *ThemeSwitchWidget) Paint(dst *ggui.Canvas, r ggui.Rect) {
	...
	dst.Shadow(box, 18, ggui.ShadowStyle{Offset: ggui.Pt(0, 2), Blur: 3,
		Color: fade(color.Black, pick(s.IsInert(), .03, .07))})
	...
	stars := fade(color.White, k*pick(s.IsInert(), .3, .9))
	...
	cloud := fade(color.White, (1-k)*pick(s.IsInert(), .3, .8))
	...
	dst.FillCircle(center, 16, fade(body, pick(s.IsInert(), .04, .1)))
	...
	crater := fade(mix(body, color.NRGBA{R: 223, G: 211, B: 222, A: 255}, .4), k*pick(s.IsInert(), .3, .65))
