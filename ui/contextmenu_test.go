...
func TestContextMenuPointerKeyboardAndAccessibility(t *testing.T) {
	...
	p.ClickButton(target.Center(), ebiten.MouseButtonRight)
	...
	p.Type(ggui.Mods{Shift: true}, ebiten.KeyF10)
	if !menu.Popup().IsOpen() {
		t.Fatal("focus restoration / Shift+F10")
	}
	p.Type(ggui.Mods{}, ebiten.KeyHome, ebiten.KeyArrowUp, ebiten.KeyEnter)
	if picked != "delete" {
...
func TestContextMenuNoEnabledActions(t *testing.T) {
	for _, entries := range [][]ggui.Widget{nil, {ui.MenuItem("Disabled", func() { t.Fatal("disabled action ran") }).Disabled(true)}} {
		...
		p.Type(ggui.Mods{}, ebiten.KeyArrowUp, ebiten.KeyArrowDown, ebiten.KeyHome, ebiten.KeyEnd, ebiten.KeyEnter)
		p.Type(ggui.Mods{}, ebiten.KeyEscape)
		if menu.Popup().IsOpen() {
...
func TestContextMenuEdgePlacementAndRebuild(t *testing.T) {
	...
	defer p.Close()
	p.ClickButton(ggui.Pt(298, 198), ebiten.MouseButtonRight)
	version.Set(1)
	p.Frame()
	r := menu.Popup().Rect()
