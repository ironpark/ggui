...
platform's own Open, Save or folder dialog through the `runtime` package.
...
reports `runtime.ErrUnsupported`. The test answers them with a
`runtime.StubFilePicker` and drops in-memory files through `Probe.Drop`, so it
runs headlessly:
